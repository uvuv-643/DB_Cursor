...
# CONFIG
# =========================
DB_BACKEND = "sqlite"  # "postgres" or "sqlite"
...
def introspect_postgres():
    ...
    for row in fks:
        ...
        block["on_delete"] = decode_action(row["on_delete_rule"])
        block["on_update"] = decode_action(row["on_update_rule"])
...
def print_schemas(schemas):
    for schema_name, tables_map in sorted(schemas.items()):
        ...
        for table_name, meta in sorted(tables_map.items()):
            ...
            if meta.get("pk"):
                print("  PRIMARY KEY:")
                print(f"    ({', '.join(meta['pk'])})")

            uniques = meta.get("uniques", {})
            if uniques:
                print("  UNIQUE CONSTRAINTS / UNIQUE INDEXES:")
                for cname, cols_list in uniques.items():
                    print(f"    {cname}: ({', '.join(cols_list)})")
            ...
            if fks:
                ...
                for fk in fks:
                    ...
                    if "." not in tgt_desc and schema_name != "main":
                        ...
                        tgt_full = tgt_desc
                    print(f"    {fk['name']}: ({', '.join(fk['src_cols'])}) -> {tgt_full}({', '.join(fk['tgt_cols'])})")
                    if fk.get("on_update") or fk.get("on_delete"):
                        print(f"        ON UPDATE {fk.get('on_update')} ON DELETE {fk.get('on_delete')}")

            idxs = meta.get("indexes", [])
            if idxs:
                print("  INDEXES:")
                for idx in idxs:
                    print(f"    - {idx['name']}: {idx['def']}")
