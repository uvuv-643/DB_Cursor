...
Формат SQL-запросов должен СТРОГО соответствовать следующим правилам:
- начинается с SELECT * FROM
- далее идет последовательный JOIN таблиц через ON
- использование ТОЛЬКО WHERE для фильтрации (без GROUP BY, HAVING, ORDER BY, LIMIT, OFFSET)
Если полученный запрос содержит HAVING, GROUP BY, ORDER BY, LIMIT или OFFSET - удали эти части из запроса.

Формат ответа: JSON с ключами:
- "partial_queries": dict[str, str] — SQL-запросы по каждому фрагменту (начинаются с WHERE)
- "query_head": str — начало SQL-запроса (до WHERE)


Если сформировать запрос невозможно, выдай пустой JSON с ключами "partial_queries" и "query_head".
